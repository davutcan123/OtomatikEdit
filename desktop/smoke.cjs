...
exports.run = async ({ mainWindow, origin, token, dataDir, health }) => {
  ...
  const parallelBefore=await mainWindow.webContents.executeJavaScript(`(async()=>{
    ...
    const sourceClipId=second.clipId;if(!timelineCopySelection())throw new Error('Packaged timeline copy failed');createTimeline();const copyTimeline=S.activeTimelineId;S.timelines.find(item=>item.id===copyTimeline).name='Kopyalanmış kısa video';addProjectAssetAt(source,0);addVideoTrack();seekOutputTime(.35);const pasted=timelinePasteClipboard();if(!pasted)throw new Error('Cross-timeline paste failed');
  ...
  const parallelAfter=await mainWindow.webContents.executeJavaScript(`(async()=>{
    // A newly created canvas is already 300x150. Wait for the requested
    // decoded frame and its actual paint, not that default buffer size.
    seekOutputTime(.9);const deadline=performance.now()+10000;while(performance.now()<deadline){const nodes=[...document.querySelectorAll('#video-composite .composite-layer')];if(nodes.length===3&&nodes.every(node=>{const v=node.querySelector('video'),c=node.querySelector('canvas'),clip=S.clips.find(item=>item.clipId===node.dataset.clipId),expected=clip?.start+(.9-clip?.timelineStart);return!!clip&&!node.hidden&&v.readyState>=2&&!v.seeking&&Math.abs(v.currentTime-expected)<.04&&c.dataset.sourceTime!==undefined&&Math.abs(Number(c.dataset.sourceTime)-expected)<.04&&c.width>0&&c.height>0}))break;await new Promise(resolve=>setTimeout(resolve,25))}
